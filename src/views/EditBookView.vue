    ...
    <div class="col-lg-8">
      <div class="card border-0 shadow-sm">
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
          <h2 class="h4 mb-0">Editar Livro</h2>
          <RouterLink to="/pesquisar-livro" class="btn btn-outline-secondary">
        ...
        <div class="card-body p-4">
          ...
          <form v-if="book" @submit.prevent="promptForPasswordConfirmation">
            <div class="mb-3">
          ...
          <div v-else class="alert alert-danger">
            Livro não encontrado ou carregando...
          </div>
        </div>
      </div>
    </div>
...
onMounted(() => {
  ...
  const foundBook = bookStore.getBookById(bookId);
  if (foundBook) {
...
const handleActualUpdate = () => {
  if (book.value) {
    bookStore.editBook(book.value.id, book.value);
